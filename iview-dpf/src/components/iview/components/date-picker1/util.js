    ...
    date: {
        formatter(value, format) {
            if (value && value[0]!=null) {
                const trueDate = new Date(value[0]);
                let month = trueDate.getMonth();
                let dateStr = ''
                if (month == 0) {
                    dateStr = trueDate.getFullYear() + '-' + 1 + '-' + trueDate.getDate()
                } else {
        ...
        formatter(value, format) {
            if (value && value[0]!=null && value[1]!=null) {
                ...
                for (let i=0;i<value.length;i++) {
                    const trueDate = new Date(value[i]);
                    let month = trueDate.getMonth();
                    let dateStr = ''
                    if (month == 0) {
                        dateStr = trueDate.getFullYear() + '-' + 1 + '-' + trueDate.getDate()
                ...
                return reyurnStr[0] + ' - ' + reyurnStr[1];
            }
        },
        parser(text, format) {
            // parse as if a normal date
